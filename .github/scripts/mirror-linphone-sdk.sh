...
if [ -f "$sdk/.gitmodules" ] || [ ! -f "$sdk/MIRROR_SOURCE.txt" ]; then
  ...
  # Capture the real version while git metadata still exists. Flattening destroys the
  # tags, so `git describe` cannot run in CI -- the workflow must pass this value as
  # -DLINPHONESDK_VERSION or bctoolbox aborts with "fail to get GIT describe version".
  sdk_describe=$(git -C "$sdk" describe 2>/dev/null || echo "unknown")
  sdk_semver=$(printf '%s' "$sdk_describe" | sed -n 's/^\([0-9]\+\.[0-9]\+\.[0-9]\+\).*/\1/p')
  [ -n "$sdk_semver" ] || sdk_semver="unknown"
  echo "==> SDK version from git describe: $sdk_describe (semver: $sdk_semver)"
...
source:      $SDK_URL
commit:      $sdk_sha
describe:    $sdk_describe
semver:      $sdk_semver

Pass -DLINPHONESDK_VERSION=$sdk_semver when configuring: this snapshot has no tags,
so bc_compute_full_version() cannot derive a version and aborts the CMake configure.
